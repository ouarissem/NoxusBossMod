...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float uvOffsetAngle = tex2D(uImage1, coords) * 16 + uTime * 9;
    float2 uvOffset = float2(sin(uvOffsetAngle + 1.57), sin(uvOffsetAngle)) * uIntensity * 0.0016;
    ...
    float4 color = tex2D(uImage0, coords + uvOffset * distanceFadeoff);
