    ...
    public class PrimitiveTrailCopy
    {
        ...
        internal void DrawPrimsFromVertexData(List<VertexPositionColorTexture> vertices, List<short> triangleIndices, Vector2 offset)
        {
            ...
            if (SpecialShader != null)
            {
                ...
                SpecialShader.Shader.Parameters["uWorldViewProjection"].SetValue(fuck);
                SpecialShader.Shader.Parameters["uCorrectionOffset"].SetValue(offset / new Vector2(Main.screenWidth, Main.screenHeight) * Main.GameViewMatrix.Zoom * 2f);
                SpecialShader.Apply();
            }
            else
