...
namespace NoxusBossMod.Content.Particles
{
    public class NoxusEggDecalParticle : Particle
    {
        ...
        public override void CustomDraw(SpriteBatch spriteBatch)
        {
            ...
            var backgroundShader = GameShaders.Misc[$"{NoxusBossMod.Instance.Name}:MonochromeShader"];
            backgroundShader.Shader.Parameters["contrastInterpolant"].SetValue(contrastInterpolant);
            backgroundShader.Apply();

            Texture2D texture = ModContent.Request<Texture2D>("NoxusBossMod/Content/Bosses/NoxusEgg").Value;
