...
using NoxusBossMod.Common.Utilities;
using NoxusBossMod.Content.Items;
using NoxusBossMod.Content.Items.Pets;
using NoxusBossMod.Content.Particles;
...
namespace NoxusBossMod.Content.Bosses
{
    ...
    public class EntropicGod : ModNPC
    {
        ...
        public static readonly SoundStyle BrainRotSound = new("NoxusBossMod/Assets/Sounds/Custom/NoxusBrainRot");

        public static readonly SoundStyle ClapSound = new SoundStyle("NoxusBossMod/Assets/Sounds/Custom/NoxusClap") { Volume = 1.5f };

        public static readonly SoundStyle ExplosionSound = new("NoxusBossMod/Assets/Sounds/Custom/NoxusExplosion");

        public static readonly SoundStyle ExplosionTeleportSound = new("NoxusBossMod/Assets/Sounds/Custom/NoxusExplosionTeleport");

        public static readonly SoundStyle FireballShootSound = new SoundStyle("NoxusBossMod/Assets/Sounds/Custom/NoxusFireballShoot") { Volume = 0.65f, MaxInstances = 20 };

        public static readonly SoundStyle HitSound = new SoundStyle("NoxusBossMod/Assets/Sounds/NPCHit/NoxusHurt") { PitchVariance = 0.4f, Volume = 0.5f };

        public static readonly SoundStyle JumpscareSound = new("NoxusBossMod/Assets/Sounds/Custom/NoxusJumpscare");

        public static readonly SoundStyle NightmareDeathrayShootSound = new SoundStyle("NoxusBossMod/Assets/Sounds/Custom/NoxusNightmareDeathray") { Volume = 1.56f };

        public static readonly SoundStyle ScreamSound = new SoundStyle("NoxusBossMod/Assets/Sounds/Custom/NoxusScream") { Volume = 0.45f, MaxInstances = 20 };

        public static readonly SoundStyle TwinkleSound = new SoundStyle("NoxusBossMod/Assets/Sounds/Custom/NoxusTwinkle") { MaxInstances = 5, PitchVariance = 0.16f };
        ...
        public override void SetStaticDefaults()
        {
            ...
            if (ModLoader.TryGetMod("BossChecklist", out Mod bossChecklistMod))
            {
                try
                {
                    Texture2D bossIcon = ModContent.Request<Texture2D>("NoxusBossMod/Content/Bosses/EntropicGodIcon").Value;
                    Texture2D bossTexture = ModContent.Request<Texture2D>("NoxusBossMod/Content/Bosses/EntropicGod").Value;

                    Action<SpriteBatch, Rectangle, Color> portraitDrawer = (sb, rect, color) =>
                    {
                        sb.Draw(bossIcon, rect, color);
                    };

                    Action<SpriteBatch, Rectangle, Color> bossPortraitDrawer = (sb, rect, color) =>
                    {
                        Vector2 scale = new Vector2(rect.Width / (float)bossTexture.Width, rect.Height / (float)bossTexture.Height);
                        sb.Draw(bossTexture, rect, null, color, 0f, Vector2.Zero, scale, 0, 0f);
                    };
        ...
        public void DoBehavior_DarkEnergyBoltHandWave()
        {
            ...
            int telegraphTime = 45; // NoxSpike.TelegraphTime estimated
            ...
            if (AttackTimer >= DefaultTeleportDelay * 2f + handArcTime + telegraphTime - 5f)
            {
                if (AttackTimer == DefaultTeleportDelay * 2f + handArcTime + telegraphTime)
                {
                    ...
                    // ScreenEffectSystem.SetBlurEffect replaced with manual screen shake
        ...
        // Helper method for projectile spawning (simplified)
        public static int NewProjectileBetter(Vector2 position, Vector2 velocity, int type, int damage, float knockback, int owner = -1, float ai0 = 0f, float ai1 = 0f)
        {
            if (owner == -1) owner = Main.myPlayer;
            return Projectile.NewProjectile(Projectile.GetSource_NaturalSpawn(), position, velocity, type, damage, knockback, owner, ai0, ai1);
        }
        ...
        public void TeleportTo(Vector2 teleportPosition)
        {
            ...
            // FIX 2026: Providence.NearBurnSound - check if still exists, fallback to SoundID.Item8
            try
            {
                SoundEngine.PlaySound(Providence.NearBurnSound with { Pitch = 0.5f, Volume = 2f, MaxInstances = 8 }, NPC.Center);
            }
            catch
            {
                SoundEngine.PlaySound(SoundID.Item8 with { Pitch = 0.5f, Volume = 2f }, NPC.Center);
            }
        ...
        #region Remaining Attack Methods
        public void DoBehavior_RealityWarpSpinCharge()
        {
            ...
            else if (AttackTimer <= teleportDelay + spinDelay)
            {
                if (AttackTimer == teleportDelay + 1f)
                {
                    ...
                    ScreenShatterSystem.TriggerShatter(NPC.Center);
            ...
            else if (AttackTimer < DefaultTeleportDelay * 2f + handRaiseTime + portalCastTime)
            {
                ...
                if (AttackTimer == DefaultTeleportDelay * 2f + handRaiseTime + 4f)
                {
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        NewProjectileBetter(portalSpawnPosition + Target.velocity * 30f, (PortalArcSpawnCenter - portalSpawnPosition).SafeNormalize(Vector2.UnitY), ModContent.ProjectileType<DarkPortal>(), 0, 0f, -1, portalScale, portalLingerTime);
        ...
        #endregion Remaining Attack Methods

        #region More Attack Methods
        ...
        #endregion More Attack Methods

        #region Phase 3 and Final Attacks
        ...
        public void DoBehavior_DeathAnimation()
        {
            ...
            // Close the HP bar.
            // NPC.Calamity().ShouldCloseHPBar = true; // Removed - property does not exist in 2026
        ...
        #endregion Phase 3 and Final Attacks
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            Texture2D handTexture = ModContent.Request<Texture2D>("NoxusBossMod/Content/Bosses/EntropicGodHand").Value;
            InitializeHandsIfNecessary();
            ...
            if (ChargeAfterimageInterpolant > 0f)
            {
                ...
                for (int i = 60; i >= 0; i--)
                {
                    ...
                    Color afterimageColor = Color.Lerp(new Color(209, 155, 218, 0), Color.Black, Abs(ZPosition) * 0.35f) * afterimageOpacity;
        ...
        public void DrawBody(Vector2 drawPosition, Color color, float rotation, float horizontalRibOffsetTime)
        {
            ...
            for (int i = 3; i >= 1; i--)
            {
                ...
                Texture2D ribsTexture = ModContent.Request<Texture2D>($"NoxusBossMod/Content/Bosses/EntropicGodRibs{i}").Value;
        ...
        public void DrawBack(Vector2 drawPosition, Color color, float rotation)
        {
            ...
            Texture2D backTexture = ModContent.Request<Texture2D>("NoxusBossMod/Content/Bosses/EntropicGodBack").Value;
        ...
        public void DrawHead(Vector2 drawPosition, Color color, float rotation)
        {
            ...
            // Draw the head.
            Texture2D headTexture = ModContent.Request<Texture2D>("NoxusBossMod/Content/Bosses/EntropicGodHead").Value;
            ...
            if (BigEyeOpacity > 0f)
            {
                ...
                Texture2D eyeTexture = ModContent.Request<Texture2D>("NoxusBossMod/Content/Bosses/NoxusEye").Value;
        ...
        public void DrawLaserTelegraphZone()
        {
            ...
            Texture2D telegraphBorderTexture = ModContent.Request<Texture2D>("NoxusBossMod/Content/Bosses/LaserTelegraphBorder").Value;
        ...
        public override void ModifyHitByItem(Player player, Item item, ref NPC.HitModifiers modifiers)
        {
            ...
            modifiers.FinalDamage *= (float)damageReductionFactor;
        }
