...
namespace NoxusBossMod.Content.NPCs
{
    public class NoxusEggCutscene : ModNPC
    {
        ...
        public override string Texture => "NoxusBossMod/Content/Bosses/NoxusEgg";
        ...
        {
            // DisplayName removed - will be set via localization
        ...
        public void DoBehavior_ComeOutOfSky()
        {
            ...
            Rectangle actualHitbox = CenteredRectangle(NPC.Center, NPC.Size * NPC.scale);
            if (AITimer >= crashDelay && actualHitbox.Bottom >= PlayerToFollow.Bottom.Y + 8f && NoxusEgg.TileCollision(actualHitbox.BottomLeft() - Vector2.UnitY * NPC.scale * 108f, NPC.width, NPC.scale * 108f, out _) && NPC.velocity.Y != 0f)
            {
        ...
        public void DoBehavior_GetUp()
        {
            ...
            float riseInterpolant = GetLerpValue(moveDelay + 1f, riseTime, AITimer, true);
            float hoverInterpolant = GetLerpValue(riseTime + 10f, riseTime + 36f, AITimer, true);
        ...
        public void DoBehavior_DoGlitchEffectsAndDisappear()
        {
            ...
            if (Main.rand.NextBool(glitchChance) && (NoxusSky.SkyIntensityOverride <= 0.4f || glitchChance <= 1))
            {
                ...
                if (AITimer >= glitchMaximizationDelay + 120f)
                {
                    ...
                    ExpandingGreyscaleCircleParticle circle = new(NPC.Center, Vector2.Zero, new Color(219, 194, 229), 10, 0.28f);
                    VerticalLightStreakParticle bigLightStreak = new(NPC.Center, Vector2.Zero, new Color(228, 215, 239), 10, new Vector2(2.4f, 3f));
                    MagicBurstParticle magicBurst = new(NPC.Center, Vector2.Zero, new Color(150, 109, 219), 12, 0.1f);
                    for (int i = 0; i < 30; i++)
                    {
                        ...
                        VerticalLightStreakParticle smallLightStreak = new(smallLightStreakSpawnPosition, smallLightStreakVelocity, Color.White, 10, new Vector2(0.1f, 0.3f));
        ...
        {
            Color baseColor = Color.Lerp(Color.DarkGray, Color.White, GetLerpValue(0.45f, 0.875f, NPC.scale, true));
